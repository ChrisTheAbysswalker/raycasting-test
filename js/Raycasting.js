class Raycasting {
  constructor(ctx, level, x, y, playerAngle, angleIncrement, column, mapTexture) {
    this.ctx = ctx;
    this.level = level;
    this.x = x;
    this.y = y;
  ...
  castRay() {
    ...
    if (distH < distV) {
      ...
      this.pixelTexture = this.wallHitX - (Math.floor(this.wallHitX / tileSize) * tileSize);
      ...
      this.pixelTexture = this.wallHitY - (Math.floor(this.wallHitY / tileSize) * tileSize);
    }

    const relativeAngle = this.playerAngle - this.centralAngle;
    this.distance = this.distance * Math.cos(relativeAngle);
